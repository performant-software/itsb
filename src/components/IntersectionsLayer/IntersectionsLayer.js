...
const toMapData = (p, graph) => {
  ...
  const data = Object.entries(presence).map(([placeId, calendar]) => {
    ...
    const intersections = Object.entries(calendar).filter(([month, present]) => {
      if (present.length < 2)
        return false; // Only one presence
...
export const IntersectionsLayer = (itineraries, graph) => {
  ...
  return [1, 2, 3].map(idx => new ScatterplotLayer({
    id: `scatterplot-${idx}`,
    data,
    pickable: true,
    opacity: 0.8,
